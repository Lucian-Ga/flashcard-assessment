...
export const AddCardButton = ({ deckId, onClickHandler }) => {
  return (
    <Link to={`/decks/${deckId}/cards/new`}>
      <button className="btn btn-primary m-2" onClick={onClickHandler}>
        + Add Cards
      </button>
    </Link>
  );
...
export const CreateDeckButton = () => {
  return (
    <Link to={"/decks/new"}>
      <button className="btn btn-secondary btn-lg m-2">+ Create Deck</button>
    </Link>
  );
};
...
export const ViewButton = ({ deckId }) => {
  return (
